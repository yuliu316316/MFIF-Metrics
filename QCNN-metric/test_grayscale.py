...
def cosine_similarity(x, y):
    num = np.dot(x, y.T)
    de_nom = np.linalg.norm(x, ord=2) * np.linalg.norm(y, ord=2)
    return num / de_nom


def lamda(x, y):
    con_x = (np.sum((x - np.mean(x)) * (x - np.mean(x)))) / (len(x) - 1)
    con_y = (np.sum((y - np.mean(y)) * (y - np.mean(y)))) / (len(y) - 1)
    l = con_x / (con_x + con_y)
    return l, con_x, con_y
...
def evaluate_Qcnn(file_path, type, samples):
    Q_data = openpyxl.load_workbook('g_Q_cnn.xlsx')
    sheet_score = Q_data['Sheet1']
    q = []
    for num in tqdm(range(1, samples+1)):
        tic = time.time()
        path1 = file_path + '/g{:02}_1.'.format(num) + type
        path2 = file_path + '/g{:02}_2.'.format(num) + type
        path3 = file_path + '/g{:02}_F.'.format(num) + type
        ...
        if h1 >= 13 and w1 >= 13:
            ...
            out_x1_1 = torch.squeeze(unfold(out_x1_1)).T.detach().cpu().numpy()
            out_x2_1 = torch.squeeze(unfold(out_x2_1)).T.detach().cpu().numpy()
            out_x3_1 = torch.squeeze(unfold(out_x3_1)).T.detach().cpu().numpy()
        else:
            p = (13 - min(h1, w1)) // 2 + 1
        ...
        if h2 >= 13 and w2 >= 13:
            unfold = torch.nn.Unfold(kernel_size=13)
            out_x1_2 = torch.squeeze(unfold(out_x1_2)).T.detach().cpu().numpy()
            out_x2_2 = torch.squeeze(unfold(out_x2_2)).T.detach().cpu().numpy()
            out_x3_2 = torch.squeeze(unfold(out_x3_2)).T.detach().cpu().numpy()
        else:
            p = (13 - min(h2, w2)) // 2 + 1
            unfold = torch.nn.Unfold(kernel_size=13, padding=p)
            out_x1_2 = torch.squeeze(unfold(out_x1_2)).T.detach().cpu().numpy()
        ...
        if h3 >= 13 and w3 >= 13:
            unfold = torch.nn.Unfold(kernel_size=13)
            out_x1_3 = torch.squeeze(unfold(out_x1_3)).T.detach().cpu().numpy()
            out_x2_3 = torch.squeeze(unfold(out_x2_3)).T.detach().cpu().numpy()
            out_x3_3 = torch.squeeze(unfold(out_x3_3)).T.detach().cpu().numpy()
        else:
            p = (13 - min(h3, w3)) // 2 + 1
            unfold = torch.nn.Unfold(kernel_size=13, padding=p)
            out_x1_3 = torch.squeeze(unfold(out_x1_3)).T.detach().cpu().numpy()
        ...
        if h4 >= 13 and w4 >= 13:
            unfold = torch.nn.Unfold(kernel_size=13)
            out_x1_4 = torch.squeeze(unfold(out_x1_4)).T.detach().cpu().numpy()
            out_x2_4 = torch.squeeze(unfold(out_x2_4)).T.detach().cpu().numpy()
            out_x3_4 = torch.squeeze(unfold(out_x3_4)).T.detach().cpu().numpy()
        else:
            p = (13 - min(h4, w4)) // 2 + 1
            unfold = torch.nn.Unfold(kernel_size=13, padding=p)
            out_x1_4 = torch.squeeze(unfold(out_x1_4)).T.detach().cpu().numpy()
        ...
        out_x1_1 = norm(out_x1_1)
        out_x1_2 = norm(out_x1_2)
        out_x1_3 = norm(out_x1_3)
        out_x1_4 = norm(out_x1_4)
        out_x2_1 = norm(out_x2_1)
        out_x2_2 = norm(out_x2_2)
        out_x2_3 = norm(out_x2_3)
        out_x2_4 = norm(out_x2_4)
        out_x3_1 = norm(out_x3_1)
        out_x3_2 = norm(out_x3_2)
        out_x3_3 = norm(out_x3_3)
        out_x3_4 = norm(out_x3_4)
        ...
        Q_data.save("g_Q_cnn.xlsx")


if __name__ == '__main__':
    evaluate_Qcnn('images/grayscale', 'tif', 1)
